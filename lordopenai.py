...
img_dir = helpers.create_path(folder="lord chatgpt")
img_save_dir = helpers.create_path(folder="lord chatgptUpscale")
...
PROMPT = (
    "Add to this image more of this characters body. Reveal up to 70% of the characters full body art, as if it was posed exactly as they are in the source image. Output should match the source image where they overlap as closely as possible,  INCLUDING transparent pixels (alpha channel). Output result must have transparent pixels. As far as image style, the output should match the following art description: Create a full-body hero character rendered as premium 2D game splash art. Use a cell shaded color style, matching the source image. Be sure to include all highlights from source image. Use sharp edges and lines. Angular shape design, clean contour hierarchy, sculpted planar anatomy, and precise facial rendering. Render shadows as intentional graphic shapes rather than blended paint. Use smooth controlled gradients only inside clearly defined forms. Do not use textured painting methods/style. Make the image lean towards sharpness. For facial features, reuse the source image to the fullest exteny possible."
)
...
PROMPT = (
    "Outpaint the supplied character artwork downward and outward to reveal "
    "approximately 70% of the character. Continue the pose, body direction, "
    "proportions, perspective, costume, lighting, and design implied by the "
    "visible source artwork.\n\n"

    "EDIT BOUNDARY: Treat every existing visible source pixel as locked reference "
    "content. Do not redraw, repaint, restyle, sharpen, soften, reinterpret, or "
    "replace any visible portion of the supplied character. Generate only the "
    "missing character content beyond the current visible artwork and within the "
    "transparent expansion area. Keep all unused background areas transparent.\n\n"

    "STYLE REFERENCE: The supplied image is the authoritative and exclusive visual "
    "style reference. Copy its rendering language closely into the newly generated "
    "body. Continue the same clean cel-shaded game illustration style, including "
    "the same sharp contour quality, simplified planar forms, smooth solid color "
    "fills, limited tonal steps, crisp shadow boundaries, clean highlights, and "
    "controlled cyan, white, orange, green, and dark-value accents.\n\n"

    "Match the source's exact level of detail. Use the same balance of broad clean "
    "shapes and selective internal line detail. Extend existing costume seams, color "
    "regions, edge lighting, hair shapes, energy effects, and geometric background "
    "elements naturally from their visible endpoints. New shapes should look as "
    "though they were drawn at the same time by the same artist using the same "
    "brushes and rendering process.\n\n"

    "Do not introduce painterly reinterpretation. Do not add visible brush strokes, "
    "canvas texture, watercolor texture, oil-painted shading, grain, mottled surfaces, "
    "rough blended edges, realistic skin texture, fabric grain, extra material texture, "
    "photorealistic lighting, complex tonal modeling, or newly invented fine detail. "
    "Do not replace the source's clean cel shading with semi-realistic digital painting.\n\n"

    "Preserve the character's visible face, hair, hand, costume, energy effects, "
    "silhouette, colors, and lighting exactly as supplied. The generated extension "
    "must visually continue the source rather than producing a redesigned version "
    "of the character."
)

PROMPT = (
    "Outpaint the supplied finished character artwork downward and outward, "
    "revealing approximately 70% of the character. Continue the pose, anatomy, "
    "proportions, perspective, costume design, lighting direction, color palette, "
    "and visual effects implied by the visible source.\n\n"

    "PROTECTED SOURCE REGION: Treat every existing visible source pixel as locked "
    "finished artwork. Do not redraw, repaint, reinterpret, restyle, resize, move, "
    "soften, sharpen, or replace the existing face, hair, hand, clothing, energy "
    "effects, geometric shapes, lighting, colors, contours, or internal shading. "
    "Generate new content only inside the editable transparent mask region.\n\n"

    "STYLE AUTHORITY: The supplied image is the exclusive visual style reference. "
    "The newly generated body must directly continue its exact rendering language. "
    "Match the source's clean stylized game-art finish, sharp controlled contours, "
    "smooth cel-shaded color regions, limited tonal steps, crisp shadow boundaries, "
    "precise internal lines, restrained gradients, simplified planar form modeling, "
    "cyan-white energy glow, orange rim accents, and low surface-texture density. Overall use the same styled highlights as found in source.\n\n"

    "Continue visible costume panels, seams, trim, color regions, shadows, highlights, "
    "hair shapes, energy effects, and geometric elements from their existing endpoints. "
    "Use the same line thickness, edge sharpness, shading complexity, color blocking, "
    "and amount of internal detail as the source. Newly generated areas should appear "
    "to have been created at the same time by the same artist using the same rendering "
    "process.\n\n"

    "Do not increase the level of detail beyond the source. Do not introduce painterly "
    "brushwork, rough digital painting, watercolor texture, oil-paint texture, canvas "
    "grain, mottled shading, visible brush strokes, realistic skin pores, fabric grain, "
    "complex material texture, noisy highlights, smeared transitions, soft undefined "
    "contours, or semi-realistic repainting. Do not convert the source's clean cel-shaded "
    "style into textured concept art.\n\n"

    "The task is to complete the missing body, not to redesign or re-render the existing "
    "artwork. Preserve the source region and extend its exact visual language into the "
    "transparent region."
)
...
def start_prompting_from_list(
    ...
    for index, filename in enumerate(filenames, start=1):
        ...
        output_filename = f"{stem}_fullbodyPrompt5.png"
