...
class Hero:
    def __init__(self, data, name, flag=False, is_alt=False, alternate_names=None):
        self.name = name
        self.role = data["role"]
        self.matchup_score = data["CounterCatScore"]
        self.teamup_names = []
        self.teamup_flags = []
        self.alternate_names = None  # always initialize consistently
        ...
        if is_alt:
            self.alternate_names = alternate_names  # store list even if empty
...
def run_counter_logic(blue, red, best, alts, replacements=None):
    teamup_dict = {}
    ...
    t = load_teamups()
    teams = build_new_classes(score_dict,t,alts)
    return teams
...
def build_new_classes(data,t,alts):
    ...
    for team in ["Blue", "New_Blue", "Alt", "Red"]:
        ...
        for name, char_data in team_data.items():
            ...
            if is_alt:
                alternate_names = alts.get(name)
                #alternate_names = alts.get(name, []) if is_alt else []
            hero = Hero(char_data, name, flag, is_alt, alternate_names)
            hero.teamup_flags = []
...
def compare_and_score_totals(team1, team2, bScoreBoth=False):
    ...
    def staty_score(team1_stat, team2_stat, fallback_base=50, scale=1.5):
        ...
        a = team1_stat if team1_stat != 0 else fallback_base
