    ...
    def build_launcher(self, parent):
        ...
        def show_ui_config():
            ...
            if not self.bConfigUI:
                ...
                other_names = config.account_config_json.get("accounts", None)
                if other_names:
                    names = self.get_account_list(other_names)
                else:
                    names = [config.USER_NAME]
                account_var = tk.StringVar()
                setname = config.account_config_json.get("set", None)
                account_var.set(setname if setname else names[0])
                ...
                account_butt.pack(side="left", fill="x", expand=True)
