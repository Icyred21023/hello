...
class PlayerFrame:
    ...
    def _build_heroes(self):
        ...
        for h in self.player.Heroes:
            if coun >= 3:
                break
            hero_obj = self.player.Heroes[h]
            coun += 1
        ...
        #     hero_obj = self._safe_at(self.top3, i, None)
            ...
            if hero != "Unknown":
                if MANUAL_DEBUG != 1:
                    ...
                    if isinstance(hero_obj.ProficiencyLevel, (str)):
                        try:
                            hero_obj.ProficiencyLevel = int(hero_obj.ProficiencyLevel)
                        except ValueError:
                            hero_obj.ProficiencyLevel = 0
                    bAnimatedLord = True if hero_obj.ProficiencyLevel >= 50 else False
                    frame = "purp" if hero_obj.ProficiencyLevel >= 30 else "gold" if hero_obj.ProficiencyLevel >= 40 else False

                    # Badge
                    if hero_obj.ProficiencyLevel >= 55:
                        badge = 4
                    elif hero_obj.ProficiencyLevel >= 45:
                        badge = 3
                    elif hero_obj.ProficiencyLevel >= 35:
                        badge = 2
                    elif hero_obj.ProficiencyLevel >= 25:
                        badge = 1
                    else:
                        badge = False
                    ...
                    # Hero icon
                    if hero_obj.ProficiencyLevel >= 50:
                        hero_icon = hero_obj.Name
                    elif hero_obj.ProficiencyLevel >= 20:
                        hero_icon = hero_obj.Name + "_l"
                    else:
                        hero_icon = hero_obj.Name
                    ...
                    # Proficiency rank
                    if hero_obj.ProficiencyLevel >= 50:
                        self.prof_rank = "Champion"
                    elif hero_obj.ProficiencyLevel >= 45:
                        self.prof_rank = "Guardian"
                    else:
    ...
    # stats canvas
        ...
        hero_timeH = hero.Stats.time_played / 3600
        usage_pct = "0%"
...
#######################################################################################
    ...
    def _buildStatTuple(self, hero: Hero, stat_names: list[str]):
        stats = []
        seconds = hero.Stats.time_played
        for stat_label in stat_names:
            if stat_label == "Games":
                stat_value = round(hero.Stats.matches_played, 1)
                stat_db = False
            elif stat_label == "Time":
                stat_value = round(hero.Stats.time_played / 3600, 1)
                stat_db = False
            elif stat_label == "Usage":
                stat_value = "0%"
                stat_db = False
            elif stat_label == "Win %":
                stat_value = hero.Stats.win_pct
                db_value = hero.Stats.win_pct_raw
                stat_db = False
                #stat_db = self.db.stat_percentile(hero.Name, "win_pct", float(db_value), player_name=getattr(self.player, "name", getattr(self.player, "player_name", None)), min_samples=10, sec = seconds)
            elif stat_label == "Mvp %":
                stat_value = hero.Stats.mvp_pct
                db_value = hero.Stats.mvp_pct_raw
                stat_db = False
                #stat_db = self.db.stat_percentile(hero.Name, "mvp_pct", float(db_value), player_name=getattr(self.player, "name", getattr(self.player, "player_name", None)), min_samples=10, sec = seconds)
            elif stat_label == "Kills":
                stat_value, db_value = hero.Stats.get_match_avg(hero.Stats.kills)
                stat_db = False
                #stat_db = self.db.stat_percentile(hero.Name, "kills_per_10", float(db_value), player_name=getattr(self.player, "name", getattr(self.player, "player_name", None)), min_samples=10, sec = seconds)
            elif stat_label == "Deaths":
                stat_value, db_value = hero.Stats.get_match_avg(hero.Stats.deaths)
                stat_db = False
                ...
                stat_value, db_value = hero.Stats.get_match_avg(hero.Stats.assists)
                stat_db = False
                ...
                stat_value, db_value = hero.Stats.get_match_avg(hero.Stats.total_damage_taken)
                stat_db = False
                ...
                stat_value = hero.Stats.accuracy
                if stat_value == "N/A":
                    stat_db = False
                else:
                    db = stat_value.strip("%") if isinstance(stat_value, str) else stat_value
                    stat_db = False
                    #stat_db = self.db.stat_percentile(hero.Name, "accuracy_pct", float(db), player_name=getattr(self.player, "name", getattr(self.player, "player_name", None)), min_samples=10, sec = seconds)
            elif stat_label == "KD/KDA":
                stat_value = round(hero.Stats.kd_ratio if hero.Role != "Strategist" else hero.Stats.kda_ratio, 2)
                db_value = hero.Stats.kd_ratio if hero.Role != "Strategist" else hero.Stats.kda_ratio
                stat_db = False
                #stat_db = self.db.stat_percentile(hero.Name, "kd_ratio" if hero.Role != "Strategist" else "kda_ratio", float(db_value), player_name=getattr(self.player, "name", getattr(self.player, "player_name", None)), min_samples=10, sec = seconds)
                stat_label = "Kd" if hero.Role != "Strategist" else "Kda"
            elif stat_label == "Damage/Healing":
                stat_value, db_value = hero.Stats.get_match_avg(hero.Stats.total_damage) if hero.Role != "Strategist" else hero.Stats.get_match_avg(hero.Stats.total_healing)
                stat_db = False
                #stat_db = self.db.stat_percentile(hero.Name, f"{hero.string.lower()}_per_minute", float(db_value), player_name=getattr(self.player, "name", getattr(self.player, "player_name", None)), min_samples=10, sec = seconds) if hero.Role != "Strategist" else self.db.stat_percentile(hero.getHeroName(), "healing_per_minute", float(db_value), player_name=getattr(self.player, "name", getattr(self.player, "player_name", None)), min_samples=10, sec = seconds)
                stat_label = "Damage" #hero.string
            else:
                stat_value = "N/A"
                stat_db = False
