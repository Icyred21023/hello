...
class SpriteSheetAnimator:
    def __init__(
        ...
        icon_wrap = tk.Frame(master, width=s(105), height=s(frame_height), bg=bg)
        icon_wrap.pack(side="left",pady=(0,0),padx=(0,0))
        ...
        cx, cy = s(52), s(54)
        self.icon_bg_img = self.createOnlyImage("_icon_bg")  # keep reference!
        self.bg_id = self.canvas.create_image(s((frame_height-3)), s(0), image=self.icon_bg_img, anchor="ne")
        ...
        if bSpecialBG:
            ...
            self.hero_bg_id = self.canvas.create_image(s(52), s((frame_height/2)), image=self.hero_bg_colored, anchor="c")
        ...
        if not self.animated:
            ...
            try:
                ...
                self.icon_bg_img = self.createOnlyImage("_icon_bg", size=(frame_height-2, frame_height-2))  # keep reference!
                self.bg_id = self.canvas.create_image(s((frame_height-3)), s(0), image=self.icon_bg_img, anchor="ne")
    ...
    def createOnlyImage1(self,player_img, size=None):
        ...
        si = img_raw.size 
        return img_raw, si
    def createOnlyImage(self,player_img, size=None):
        img_raw = image_loader(player_img)
        if not img_raw:
            return False
...
def score_player(player, hero: Hero = None):
    # ======================
    # Overall stats
    # ======================
    ...
    for hero in player.top_heroes:
        ...
        char1_score = (
            ...
            0.20 * kd1_n +
            0.20 * mvp1_n +
            0.40 * (hero.getHeroTimePlayed()/3600/50)  # pull towards overall if few games
        )
...
class PlayerFrame:
    ...
    def _create_canvas_image(
        ...
        """
        def apply_black_crops_mask(img, mask, resize_mask=False):
            """
        ...
        if mask_glow:
            img_raw = apply_black_crops_mask(img_raw, "_GlowMask", resize_mask=True)
    ...
    def _build_outer(self):
        ...
        self.outer = tk.Frame(
            self.parent,
            width=s(380),
            height=s(704) + s((self.stat_frame_height * 3)) + add,
            bg=NAME_BANNER,
            borderwidth=1,
            relief="flat",
        )
    ...
    def _build_heroes(self):
        ...
        for idx1 in self.order:  # idx1 is 1..3
            ...
            # self.hero_matches_played = matches_played
            ...
            try:
                if bAnimatedLord:
                    ...
                    anim = SpriteSheetAnimator(
                        ...
                        relief="sunken",
                        borderwidth=2,
                        frame_height = self.stat_frame_height - 4,
                        size=(108, 108),   # optional: force icon size
                    )
            ...
            self._hero_stats(parent=hero_bar, hero=hero_obj, stats_frame_size=(270, self.stat_frame_height-2), 
                              fonts = [(self.TTSuperCondMedium, 12, "normal"),("TTSupermolotNeueCond-Bd", 12, "bold")])

    def _hero_stats(self, parent=None, hero:Hero= None, label_orientation="vertical",stats_frame_size=(270, 110),  fonts = []):
    # stats canvas
        ...
        hero_name = hero_stats.create_text(s(23),s(4),tags="header",text=short_name,fill="#f1ecff",font=fonttk("Refrigerator Deluxe Heavy", 12, "bold", italic=False),anchor="nw")#"#ece5ff"#"#ece5ff"
        _,_,use,_ = self._get_iterative_coordinates(hero_stats, hero_name, padx=0, pady=0)
        m1,n1,m2,n2 = hero_stats.bbox(hero_name)
        flip = (x/2 - use) + x/2 
...
#######################################################################################
    def _buildGFX_StatLabelValue(self, canvas: tk.Canvas, scale: int = 1, label: str = None, value=None, percentile=None, x=0, y=0):
        # Create label text
        ...
        st = canvas.create_text(s(x), s(y), text=value, tags=label, fill=fgv, font=fonttk("Refrigerator Deluxe Heavy", sizev, "normal"), anchor="sw")
        x1, y1, x2, y2 = canvas.bbox(st)
        if label == "time":
                
                strp = "HR"

                canvas.create_text(x2, s(y) - 2 * scale, text=strp, tags=label, fill=fgl, font=fonttk("Refrigerator Deluxe ExtraBold", sizes, "normal"), anchor="sw")
        x = x
        y = y - int(round(18 * scale))
        l = canvas.create_text(s(x), s(y), text=label.upper() if label != "Win %" else "WIN", tags=label, fill=fgl, font=fonttk("Refrigerator Deluxe ExtraBold", sizel, "bold"), anchor="sw")

    def _buildGFX_AverageStatLabelValue(self, canvas: tk.Canvas, scale: int = 1, label: str = None, value=None, percentile=None, x=0, y=0):
        # Create label text
        #icon_label = label.lower().strip().replace(" %", "")
        #icon_label = icon_label + '2'
    ...
    def _buildStatTuple(self, hero: Hero, stat_names: list[str]):
        ...
        for stat_label in stat_names:
            if stat_label == "Games":
                ...
                stat_db = self.db.stat_percentile(hero.getHeroName(), "win_pct", float(db_value), min_samples=10)
            elif stat_label == "Mvp %":
                stat_value = hero.getHeroMvpPct()
                db_value = hero.mvp_pct_raw
                stat_db = self.db.stat_percentile(hero.getHeroName(), "mvp_pct", float(db_value), min_samples=10)
            elif stat_label == "Kills":
                stat_value, db_value = hero.getHeroStatMatchAvg(hero.kills)
                stat_db = self.db.stat_percentile(hero.getHeroName(), "kills_per_10", float(db_value), min_samples=10)
            elif stat_label == "Deaths":
                stat_value, db_value = hero.getHeroStatMatchAvg(hero.deaths)
                stat_db = self.db.stat_percentile(hero.getHeroName(), "average_lifespan", float(db_value),direction="asc", min_samples=10)
            elif stat_label == "Assists":
                stat_value, db_value = hero.getHeroStatMatchAvg(hero.assists)
                stat_db = self.db.stat_percentile(hero.getHeroName(), "assists_per_10", float(db_value), min_samples=10)
            elif stat_label == "Blocked":
                stat_value, db_value = hero.getHeroStatMatchAvg(hero.total_damage_taken)
                stat_db = self.db.stat_percentile(hero.getHeroName(), "total_damage_taken_per_minute", float(db_value), min_samples=10)
                ...
                if stat_value == "N/A":
                    ...
                    stat_db = self.db.stat_percentile(hero.getHeroName(), "accuracy_pct", float(db), min_samples=10)
                ...
                stat_db = self.db.stat_percentile(hero.getHeroName(), "kd_ratio" if hero.getHeroRole() != "Strategist" else "kda_ratio", float(db_value), min_samples=10)
                ...
                stat_db = self.db.stat_percentile(hero.getHeroName(), f"{hero.string.lower()}_per_minute", float(db_value), min_samples=10) if hero.getHeroRole() != "Strategist" else self.db.stat_percentile(hero.getHeroName(), "healing_per_minute", float(db_value), min_samples=10)
    ...
    def _setLabelValue_CanvasText1(
        ...
        l = canvas.create_text(s(x), s(y), text=lab if lab != "Win %" else "Win", tags=tag, fill=fills[0], font=fonttk(fam, siz, wei), anchor="nw")
    ...
    def build_avg_stat_ui(self,p, x, y, label, value, percentile):
        ...
        a,b,c,d = self._get_iterative_coordinates(p, icon, padx=0, pady=0)
        print(f"Icon coords for {label}:\na:{a}, \nb:{b}, \nc:{c}, \nd:{d}\n____________\n")
        ft_size = 12
        ...
        if label.lower() in ['blocked', 'damage', 'healing']:
            ft_size = 10
        l = p.create_text(s(x),s(64),text=label,fill=self.fg_l,tags=label,font=fonttk("TTSupermolotNeueCond-Md", 8, "normal"),anchor="center")
        ...
        v = p.create_text(s(x)+s(xOffset),s(100),text=vsplit1,tags=label,fill=fg,font=fonttk("Refrigerator Deluxe ExtraBold", ft_size, "normal"),anchor="center")
    ...
    def build_launcher(self, parent):
        ...
        def show_ui_config():
            ...
            if not self.bConfigUI:
                ...
                self.ui_config = tk.Frame(deb, bg="#151426", relief="solid", height=s(250), width=s(230))
                stat_labels = ["Stat 1", "Stat 2", "Stat A", "Stat B", "Stat C", "Stat D"]
