...
def find_cf_log_entry(log):
    result = None
    for entry in log:
        try:
            outer_message = json.loads(entry["message"])
            params = outer_message['message']["params"]
            ass = params.get("associatedCookies", [])
            head = params.get("headers", {})
            if len(ass) == 0:
                continue
            for item in ass:
                ...
                if cookie.get("name") == "cf_clearance":
                    ...
                    sec_ch_ua = head.get("sec-ch-ua", "")
                    ua = head.get("user-agent", "")
                    prio = head.get("priority", "")
                    return cf_clearance, sec_ch_ua, ua, prio
                    result = entry
                    return result
...
def extract_cookie_values(log_entry):
    data = log_entry
    outer_message = json.loads(data["message"])
    params = outer_message['message']["params"]
    ...
    return cf_clearance, params.get("headers", {}).get("cookie")


def _header_value_to_string(value: Any) -> str:
    """
    CDP headers are normally strings, but this safely handles
    unexpected primitive values.
    """
...
def request_to_curl(
    ...
    """
    Build a POSIX/PowerShell-friendly curl command.

    Uses curl.exe explicitly so PowerShell does not interpret `curl`
    as an alias on older Windows PowerShell installations.
    """
    command = [
        "curl.exe",
        url,
    ...
    # These are generated automatically by curl and can cause problems
    # if copied exactly from Chrome.
    ignored_headers = {
        "content-length",
        "host",
        "connection",
    ...
    for name, value in headers.items():
        ...
        header = f"{name}: {_header_value_to_string(value)}"
...
def wait_for_api_curl(
    ...
    *,
    ...
    """
    Wait for a browser request matching target_url, then return its
    complete request information and a reconstructed curl command.

    Matching:
        exact_url=False:
            target_url only needs to appear in the requested URL.

        exact_url=True:
            requested URL must equal target_url.
    ...
    if clear_existing_logs:
        # driver.get_log() drains the current performance-log queue.
        driver.get_log("performance")

    deadline = time.monotonic() + timeout

    matched_requests: dict[str, dict[str, Any]] = {}
    ...
    while time.monotonic() < deadline:
        ...
        for entry in entries:
            ...
            # The ExtraInfo event usually contains browser-generated
            # headers that requestWillBeSent may omit, especially Cookie.
            if event_name == "Network.requestWillBeSentExtraInfo":
                if request_id:
                    extra_headers[request_id] = params.get("headers", {})

                    # ExtraInfo can arrive after requestWillBeSent.
                    if request_id in matched_requests:
                        matched_requests[request_id]["extra_headers"] = (
                            params.get("headers", {})
                        )
            ...
            matches = (
                ...
                if exact_url
                else target_url in request_url
            )

            if not matches:
                continue

            matched_requests[request_id] = {
                "request": request,
                "extra_headers": extra_headers.get(request_id, {}),
            }

        if matched_requests:
            # Give requestWillBeSentExtraInfo a brief opportunity to arrive.
            extra_info_deadline = min(deadline, time.monotonic() + 0.75)

            while time.monotonic() < extra_info_deadline:
                entries = driver.get_log("performance")
            ...
            # Return the newest matching request.
            request_id = next(reversed(matched_requests))
            captured = matched_requests[request_id]
            request = captured["request"]

            regular_headers = request.get("headers", {})
            actual_headers = captured.get("extra_headers", {})

            # ExtraInfo headers are generally the headers actually sent.
            # Merge regular headers in as a fallback.
            headers = {
                **regular_headers,
                **actual_headers,
            }
            ...
            # Sometimes postData is not embedded in requestWillBeSent.
            if body is None and request.get("hasPostData"):
                try:
                    ...
                    print(
                        "Could not retrieve request POST data:",
                        repr(exc),
                    )
    ...
    raise TimeoutError(
        f"No API request matching {target_url!r} was observed "
            ...
            for t in p.Teammates:
                    for item in t.FetchedTeammates:
                            if str(item.PlayerImgId) == p.PlayerImgId:
                                ...
                                p.RealPlayer = item
                                item.convertToMatchPlayer(p)
                                self.Players.append(item)
                                break
                    if p.RealPlayer is not None:
                        break
...
def fetchPlayer(uid, driver= None):
    ...
    if driver is None:
        ...
        ressult = driver.fetch_post(URL, pl)
    return ressult
...
def fetchLive(uid, mid, driver= None):
...
def try_selenium2(u):
    ...
    Chrome = tracker_lookup.BROWSER
    tries = 0

    while True:
        try:
            Chrome.driver.get(f"https://rivalsdata.com/{u}")

            if Chrome.driver.title == "Just a moment...":
                print(
                    "SELENIUM: Captcha detected. "
                    "Please solve it in the browser window."
                )

                Chrome.set_captcha_window()
                Chrome.wait_for_captcha()
                Chrome.set_tiny_window()

            print("Cloudflare check passed.")

            # Drain requests made during page loading/CAPTCHA.
            log = Chrome.driver.get_log("performance")

            # Cause the site's JavaScript to make the API request here.
            #
            # Examples:
            ...
            #   refresh a component
            #
            # Chrome.driver.find_element(...).click()

            cf, sec_ch_ua, ua,prio = find_cf_log_entry(log)
            if cf is None:
                raise RuntimeError(
                    "Could not find Cloudflare log entry. "
            ...
            import curl
            URL = "https://api.rivalsdata.com/player"
            pl= {"uid": config.USER_UID}
            response = curl.curl_get(URL,cf,pl, ua, sec_ch_ua,prio)
            return response
            ...
            tries += 1
            print(f"Capture attempt {tries} failed:", exc)
...
def try_selenium(u):
    ...
    while True:
        try:
            ...
            Chrome.driver.get(f"https://rivalsdata.com/{u}")
            if Chrome.driver.title == "Just a moment...":
                print("SELENIUM: Captcha detected. Please solve it in the browser window.")
                Chrome.set_captcha_window()
                y = Chrome.wait_for_captcha()
                Chrome.set_tiny_window()
            #y = Chrome.wait_for_captcha()
            #if y:
                #Chrome.set_tiny_window()
            status = fetchPlayer(u, Chrome)
            status_result = parsePlayerStatus(status)
            if status_result:
                print(f"SELENIUM: Player with UID {u} is in a match with ID: {status_result}")
                break
            time.sleep(1.5)
        except Exception as e:
            if tries >= 3:
                print(f"Selenium Error occurred: {e}. Maximum retries reached. Exiting.")
                return True, None
            tries += 1
            print(f"Selenium Error occurred: {e}. Retrying in 3 seconds.")
            time.sleep(3)
    while True:
        try:
            match = fetchLive(u, status_result, Chrome)
        except Exception as e:
            if tries >= 3:
                print(f"Selenium Error occurred: {e}. Maximum retries reached. Exiting.")
                match = False
...
def getLiveMatchNames(u):
    ...
    while True:
        try:
            ...
            time.sleep(1.5)
             ...
             if '403' in str(e):
                print("RivalsData API returned 403 Forbidden. Trying Selenium method.")
                bSelenium, match = try_selenium2(u)
                
        
            
    while True:
        try:
            if bSelenium and match:
    ...
    try:
        LiveMatchObject.findPrivateTeammates()
    except Exception as e:
        print("Exception", e)
...
if __name__ == "__main__":
    ...
    result = getLiveMatchNames(config.USER_UID)
    time.sleep(199)
    tit = getLiveMatchNames(config.USER_UID)
    if DEBUG_PRIVATE:
