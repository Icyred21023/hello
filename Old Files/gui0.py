...
class Regions:
    def __init__(self, player_frame: "PlayerFrame"=None):
        self._pf = player_frame
        self.full_box = (0,0,self._pf.card_w, self._pf.card_h)
        self._regions = {}
        self.idx = 0


    def addRegion(self, parent=False, name=None, box=None):
        if not name:
            while True:
                name = f"null_{str(self.idx)}"
                if name not in self._regions:  # just in case
                    break
                self.idx += 1
        if parent and parent in self._regions:
            parent_d = self._regions[parent]
            parent_box = parent_d.get("box", self.full_box)
        box = (0,0,0,0) if box is None else box
        offbox = self.offset_boxes(box, parent_box) if parent else (box)

        self._regions[name] = {"box": offbox, "w": self._pf.box_w(offbox), "h": self._pf.box_h(offbox), "parent": parent}
        return offbox  # optional convenience
    
    # def add(self, name, box):
    #     self._regions[name] = {"box": box, "w": self._pf.box_w(box), "h": self._pf.box_h(box)}
    #     return box  # optional convenience

    def _s2(self, xy):
        """Your existing s(...) scaler expects tuples sometimes."""
        return s(xy)

    def createImage(self, canvas: "tk.Canvas"=None, img_key: str = "", region_name: str = "", x=0,y=0,xrel=None,yrel=None,anc="nw", size=None, **kwargs):
        if canvas is None or region_name not in self._regions:
            return None
        box = self._regions[region_name]["box"]
        if anc == "nw":
            regx, regy = box[0] + x, box[1] + y
        elif anc in ("center", "c"):
            regx = x + box[0] + self._pf.box_w(box) // 2
            regy = y +   box[1] + self._pf.box_h(box) // 2
        else:
            regx = box[0] + self._pf.box_w(box) if anc in ("ne", "se") else box[0]
            regy = box[1] + self._pf.box_h(box) if anc in ("sw", "se") else box[1]
        
        cx, cy = box[0], box[1]
        t= self._create_canvas_image(canvas, img_key, x=regx, y=regy, size=size)
        if t:
            bbox = canvas.bbox(t)
            self.addRegion(name=img_key, parent=region_name, box=bbox)
    ...
    def offset_boxes(self, box1, box2):
        x1, y1, x2, y2 = box1
        ox, oy = box2[0], box2[1]
...
class PlayerFrame(tk.Canvas):
    def __init__(self, parent, width, height, player: Player):
        ...
        #self.debug_draw_regions()   # remove later
        # self.draw()
        ...
        # self.nameplate = None
    def setRegions(self):
            # Define all regions and subregions
            outer = (9,10, self.card_w-10, self.card_h-11)
            header = (15,16, self.card_w-15, 60)
            overview = (17, 66, self.card_w-17, 258)
            roles = (17, 258+12, self.card_w-17, 470)
            heroes = (17, 470+12, self.card_w-17, 482+380)
            last =  (17, 482+380+12, self.card_w-17, 1117)
            ...
            self.regions = {
                ...
                "heroes": {"box": heroes},
                "last": {"box": last},
                # Subregions will be defined in compute_layout
            }

    def inset(self, box: Box, pad: int) -> Box:
        x1, y1, x2, y2 = box
        return (x1 + pad, y1 + pad, x2 - pad, y2 - pad)

    def box_w(self, box: Box) -> int:
        return box[2] - box[0]

    def box_h(self, box: Box) -> int:
        return box[3] - box[1]

    def split_h(self, box: Box, top_h: int) -> tuple[Box, Box]:
        x1, y1, x2, y2 = box
        return (x1, y1, x2, y1 + top_h), (x1, y1 + top_h, x2, y2)

    def split_v(self, box: Box, left_w: int) -> tuple[Box, Box]:
        x1, y1, x2, y2 = box
        return (x1, y1, x1 + left_w, y2), (x1 + left_w, y1, x2, y2)
    def compute_layout(self):
        pad = self.styles["outer_pad"]
        gap = self.styles["section_gap"]

        outer = (pad, pad, self.card_w - pad, self.card_h - pad)
        self.regions["outer"] = outer

        x1, y1, x2, y2 = outer
        cur_y = y1

        # Fixed heights for a tall vertical player card
        header_h = 44
        threat_h = 120
        overall_h = 120
        role_h = 88
        heroes_h = 210
        ban_h = y2 - (cur_y + header_h + gap + threat_h + gap + overall_h + gap + role_h + gap + heroes_h + gap)

        # Header
        self.regions["header"] = (x1, cur_y, x2, cur_y + header_h)
        cur_y += header_h + gap

        # Threat summary
        self.regions["threat_summary"] = (x1, cur_y, x2, cur_y + threat_h)
        cur_y += threat_h + gap

        # Overall stats
        self.regions["overall_stats"] = (x1, cur_y, x2, cur_y + overall_h)
        cur_y += overall_h + gap

        # Role info
        self.regions["role_info"] = (x1, cur_y, x2, cur_y + role_h)
        cur_y += role_h + gap

        # Top heroes
        self.regions["top_heroes"] = (x1, cur_y, x2, cur_y + heroes_h)
        cur_y += heroes_h + gap

        # Ban recommendation / notes
        self.regions["ban_recommendation"] = (x1, cur_y, x2, y2)

        # Subregions
        self.compute_threat_subregions()
        self.compute_overall_subregions()
        self.compute_role_subregions()
        self.compute_hero_subregions()

    def debug_draw_regions(self):
        self.delete("all")

        for name, box in self.regions.items():
            if name == "outer":
                self.draw_panel(box, fill=self.styles["bg_outer"], outline="#5d6b99", width=2, tags=("debug",))
            else:
                self.draw_panel(box, fill="", outline="#3f7cff", width=1, tags=("debug",))
                x1, y1, x2, y2 = box
                self.create_text(
                    x1 + 6, y1 + 6,
                    anchor="nw",
    ...
    def draw_panel(self, box: Box, fill=None, outline=None, width=1, tags=()):
        if fill is None:
            fill = self.styles["bg_panel"]
        if outline is None:
            outline = self.styles["outline"]
        ...
        return item
    def compute_role_subregions(self):
        box = self.inset(self.regions["role_info"], self.styles["inner_pad"])
        x1, y1, x2, y2 = box
        h = self.box_h(box)
        row_gap = 4
        row_h = (h - row_gap * 2) // 3

        self.regions["role_vanguard"] = (x1, y1, x2, y1 + row_h)
        self.regions["role_duelist"] = (x1, y1 + row_h + row_gap, x2, y1 + row_h * 2 + row_gap)
        self.regions["role_strategist"] = (x1, y1 + row_h * 2 + row_gap * 2, x2, y2)

    def compute_hero_subregions(self):
        box = self.inset(self.regions["top_heroes"], self.styles["inner_pad"])
        x1, y1, x2, y2 = box
        h = self.box_h(box)
        row_gap = 8
        row_h = (h - row_gap * 2) // 3

        self.regions["hero_row_1"] = (x1, y1, x2, y1 + row_h)
        self.regions["hero_row_2"] = (x1, y1 + row_h + row_gap, x2, y1 + row_h * 2 + row_gap)
        self.regions["hero_row_3"] = (x1, y1 + row_h * 2 + row_gap * 2, x2, y2)

    def compute_overall_subregions(self):
        box = self.inset(self.regions["overall_stats"], self.styles["inner_pad"])
        x1, y1, x2, y2 = box
        w = self.box_w(box)
        h = self.box_h(box)
        ...
        names = [
            "matches_box", "time_box",
            "kills_box", "deaths_box",
            "damage_box", "healing_box",
        ]

        boxes = []
        for r in range(3):
            for c in range(2):
                bx1 = x1 + c * (col_w + col_gap)
                by1 = y1 + r * (row_h + row_gap)
                bx2 = bx1 + col_w
                by2 = by1 + row_h
    ...
    def compute_threat_subregions(self):
        box = self.inset(self.regions["threat_summary"], self.styles["inner_pad"])
        x1, y1, x2, y2 = box
        w = self.box_w(box)
        h = self.box_h(box)

        left_w = int(w * 0.44)
        left_box, right_box = self.split_v(box, left_w)

        # Left stack: KDA, MVP, WIN
        lx1, ly1, lx2, ly2 = left_box
        kda_h = int((ly2 - ly1) * 0.46)
        mvp_h = int((ly2 - ly1) * 0.27)

        self.regions["kda_box"] = (lx1, ly1, lx2, ly1 + kda_h)
        self.regions["mvp_box"] = (lx1, ly1 + kda_h + 4, lx2, ly1 + kda_h + 4 + mvp_h)
        self.regions["win_box"] = (lx1, ly1 + kda_h + 4 + mvp_h + 4, lx2, ly2)

        self.regions["threat_side"] = right_box

    def center_of(self, box):
        x1, y1, x2, y2 = box
        return (x1 + x2) // 2, (y1 + y2) // 2

    def add_item(self, section, item_id):
        self.items.setdefault(section, []).append(item_id)

    def clear_section(self, section):
        for item_id in self.items.get(section, []):
            self.delete(item_id)
        self.items[section] = []
    ...
    def draw(self):
        ...
        #self.draw_top_heroes()
        #self.draw_ban_recommendation()

    def getRegionCoordinates(self, region_name):
        return self.regions.get(region_name)

    def draw_outer(self):
        self.regions.addRegion(name="outer", box=(0,0,self.card_w,self.card_h))
        self.regions.createImage(canvas=self, img_key="#outer",region_name="outer", anc="nw", size=(380, 1140), bg=self.styles["outer"].get("bg", "#101426"), tags=("outer_bg",))
    def draw_header(self):
        self.regions.addRegion(name="header", box=(10,45,370,235), parent="outer")
        self.regions.createImage(canvas=self, img_key="#header",region_name="header", anc="c",x=-180, y = -10, size=(360, 190), bg=self.styles["outer"].get("bg", "#101426"), tags=("header_bg",))
        #self._create_canvas_image(self, "#outer", anc="nw", size=(380, 1140), bg=self.styles["outer"].get("bg", "#101426"), tags=("outer_bg",))
        # ---------------------------
    # Public build entry
    # ---------------------------
    def build(self):
    ...
    def draw_overall_stats(self):
        section = "overall_stats"
        self.clear_section(section)

        self.draw_stat_box(self.regions["matches_box"], "MATCHES", self.player.full_overview.matches_played, section)
        self.draw_stat_box(self.regions["time_box"], "TIME", self.player.full_overview.time_played, section)
        self.draw_stat_box(self.regions["kills_box"], "KILLS", self.player.full_overview.kills, section)
        self.draw_stat_box(self.regions["deaths_box"], "DEATHS", self.player.full_overview.deaths, section)
        self.draw_stat_box(self.regions["damage_box"], "DAMAGE", self.player.full_overview.total_damage, section)
        self.draw_stat_box(self.regions["healing_box"], "HEAL", self.player.full_overview.total_healing, section)
    
    def draw_stat_box(self, box, label, value, section, value_color=None):
        x1, y1, x2, y2 = box
        value_color = value_color or self.styles["text"]
        ...
        self.add_item(section, bg)

        t1 = self.create_text(
            x1 + 8, y1 + 6,
            anchor="nw",
            text=label,
            fill=self.styles["muted"],
            font=("Segoe UI", 9, "bold")
        )
        self.add_item(section, t1)
    ...
    def _build_outer(self):
        ...
        self.outer = tk.Frame(
            ...
            height=s(704) + s((self.stat_frame_height * 4)) + add,
            bg=NAME_BANNER,
            borderwidth=1,
            relief="flat",
        )
    ...
    def _build_heroes(self):
        ...
        heroes_frame = tk.Frame(
            ...
            bg=NAME_BANNER,
            width=s(self.player_frame_width),
            height=s(36)+ s((self.stat_frame_height * 4)),
            relief="flat",
            borderwidth=0,
        )
        ...
        heroes_bg_canvas = tk.Canvas(heroes_frame, height=s(82)+ s((self.stat_frame_height * 4)), width=s(380), highlightthickness=0,bg=NAME_BANNER)
        ...
        self._create_canvas_image(
            ...
            img_key="herobg3",
            anc="nw",
            size=(380, 36 + self.stat_frame_height * 4),
            bg=NAME_BANNER,
            x=0,
            y=0,
        )
...
#######################################################################################
    ...
    def _buildGFX_AverageStatLabelValue(self, canvas: tk.Canvas, scale: int = 1, label: str = None, value=None, index:int=None, percentile=None, x=0, y=0):
        ...
        #v = canvas.create_text(x2 - 1*scale,y2 - 6 * scale,text=value if label != "Win %" else "WIN",tags=label,fill=self.fg_v,font=fonttk("Refrigerator Deluxe Heavy", 16, "normal"),anchor="sw")
        st = canvas.create_text(s(x), s(y), text=ui_value, tags=[label,"avg"], fill=fgvorig, font=fonttk("Refrigerator Deluxe Heavy", sizev, "normal"), anchor="c")
        ...
        if percentile is not False and percentile is not None:
            ...
            if percentile == 0:
                ...
                p = canvas.create_text(s(x+6), s(y+43), text=percentile, tags=[label,"avg"], fill=fgl2, font=fonttk("Refrigerator Deluxe ExtraBold", sizel-2, "bold", italic=False), anchor="c")
...
def create_player_frame(parent, player):
    frame = PlayerFrame(parent, s(380), s(1140), player)
    frame.build()
    return frame
...
class App:
    ...
    def build_match_overview(self, parent, players):
        ...
        for player in players:
            frame = PlayerFrame(player_frames, s(380), s(1140), player)
            frame.build()
