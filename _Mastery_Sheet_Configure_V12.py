...
class SpriteSheetEditor:
    ...
    def _build_ui(self) -> None:
        ...
        self._label(top, "Pad X").pack(side="left", padx=(8, 3))
        self._spin(top, self.pad_x_var, 0, 100000, 6).pack(side="left")
        self._label(top, "Pad Y").pack(side="left", padx=(8, 3))
        self._spin(top, self.pad_y_var, 0, 100000, 6).pack(side="left")
        ...
        self._button(coord_grid, "Apply Rect To Row", self.apply_rect_to_row).grid(row=0, column=5, rowspan=2, padx=4, sticky="ns")
        self._button(coord_grid, "Reset Cell", self.reset_current_rect).grid(row=0, column=6, rowspan=2, padx=4, sticky="ns")
        ...
        self._button(offset_frame, "Apply Offset To Row", self.apply_offset_to_row).pack(side="left", padx=(0, 8))
    ...
    def build_grid(self) -> None:
        ...
        try:
            ...
            pad_x, pad_y = int(self.pad_x_var.get()), int(self.pad_y_var.get())
        except Exception:
            messagebox.showerror("Invalid grid", "Rows, columns, Pad X, and Pad Y must be whole numbers.")
            return
        if rows <= 0 or cols <= 0:
            messagebox.showerror("Invalid grid", "Rows and columns must be positive.")
            return
        if pad_x < 0 or pad_y < 0:
            messagebox.showerror("Invalid grid", "Pad X and Pad Y must be zero or greater.")
            return
        usable_w = self.original_image.width - pad_x
        usable_h = self.original_image.height - pad_y
        if usable_w <= 0 or usable_h <= 0:
            messagebox.showerror("Invalid grid", "Padding leaves no usable image area.")
            return
        if rows > usable_h or cols > usable_w:
            messagebox.showerror(
                "Invalid grid",
                "Rows/columns cannot exceed the remaining usable area after padding."
            )
            return
        self.stop_animation()
        xs = [pad_x + v for v in self.integer_bounds(usable_w, cols)]
        ys = [pad_y + v for v in self.integer_bounds(usable_h, rows)]
        self.grid_rows = rows
        self.grid_cols = cols
        self.grid_pad_x = pad_x
        self.grid_pad_y = pad_y
        self.cells = []
        idx = 0
        for r in range(rows):
            for c in range(cols):
        ...
        self.select_frame(0)
        self.status_var.set(
            f"Built {rows}×{cols} grid = {len(self.cells)} frames with integer-only boundaries "
            f"starting at Pad X={pad_x}, Pad Y={pad_y}."
        )

    def default_rect_for_cell(self, cell: Cell) -> tuple[int, int, int, int]:
        usable_w = self.original_image.width - getattr(self, "grid_pad_x", 0)
        usable_h = self.original_image.height - getattr(self, "grid_pad_y", 0)
        xs = [getattr(self, "grid_pad_x", 0) + v for v in self.integer_bounds(usable_w, self.grid_cols)]
        ys = [getattr(self, "grid_pad_y", 0) + v for v in self.integer_bounds(usable_h, self.grid_rows)]
    ...
    def apply_rect_to_row(self) -> None:
        if not self.cells:
            return
        try:
            x0, y0, x1, y1 = map(int, (self.x0_var.get(), self.y0_var.get(), self.x1_var.get(), self.y1_var.get()))
        ...
        selected = self.cells[self.selected_index]
        default_x0, default_y0, default_x1, default_y1 = self.default_rect_for_cell(selected)

        # Apply the selected frame's rectangle ADJUSTMENT, not its absolute
        # sheet coordinates. Every cell in the row keeps its own grid location.
        delta_x0 = x0 - default_x0
        delta_y0 = y0 - default_y0
        delta_x1 = x1 - default_x1
        delta_y1 = y1 - default_y1
        ...
        for c in self.cells:
            ...
            base_x0, base_y0, base_x1, base_y1 = self.default_rect_for_cell(c)
            new_x0 = base_x0 + delta_x0
            new_y0 = base_y0 + delta_y0
            new_x1 = base_x1 + delta_x1
            new_y1 = base_y1 + delta_y1

            if not self.validate_rect(new_x0, new_y0, new_x1, new_y1):
                self.status_var.set(
                    f"Row apply cancelled: frame {c.index + 1} would leave the source image bounds."
                )
                return

            proposed.append((c, new_x0, new_y0, new_x1, new_y1))

        for c, new_x0, new_y0, new_x1, new_y1 in proposed:
            c.x0, c.y0, c.x1, c.y1 = new_x0, new_y0, new_x1, new_y1
            self.update_tree_row(c)

        # Keep the selected frame editor synced to the applied result.
        self.select_frame(self.selected_index)
        self.status_var.set(
            f"Applied rect adjustment Δ({delta_x0}, {delta_y0}, {delta_x1}, {delta_y1}) "
            f"to row {target_row + 1}."
        )
    ...
    def apply_offset_to_row(self) -> None:
        if not self.cells:
            return
        if self.playing:
            messagebox.showinfo("Pause first", "Pause the animation before adjusting frame position.")
        ...
        try:
            ...
            messagebox.showerror("Invalid values", "Offsets must be whole numbers.")
            return
        target_row = self.cells[self.selected_index].row
        for c in self.cells:
            if c.row == target_row:
                c.dx, c.dy = dx, dy
                self.update_tree_row(c)
        self.refresh_preview(); self.refresh_sheet_view()
        self.status_var.set(f"Applied local offset ({dx}, {dy}) to row {target_row + 1}.")
