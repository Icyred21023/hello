...
class PlayerFrame:
    ...
    def _build_outer(self):
        ...
        a += 1 if self.player.bHeroes else 0
        if self.player.bPrivate:
            
            self.superframe.createSuperFrameImage(img_key="private", x=self.x+51, y=self.y+399, anc="nw")


        elif a == 3:
            self.superframe.createSuperFrameImage(img_key="success", x=self.x+43, y=self.y+388, anc="nw")
    ...
    def _build_heroes_new(self):
        ...
        for hero in heroes:
            ...
            self.superframe.createSuperFrameImage(img_key="winrate3", x=x2, y=yb-1, anc="c")#, size=(25, 24))
            ...
            self.superframe.createSuperFrameImage(img_key="kd3", x=x3, y=yb + ykd-1, anc="c")#, size=(34, 34))
            ...
            self.superframe.createSuperFrameImage(img_key="mvp3", x=x4 + xmvp, y=yb-1, anc="c")#, size=(41, 17))
            ...
            self.superframe.createSuperFrameImage(img_key="matches2", x=x1, y=yb+1, anc="c")#, size =(22,22))
    ...
    def _build_match_history(self):
        ...
        matches = deque(self.player.matches)
        while matches and complete < 5:
            ...
            t_fg = "#b34577" if match.result == "loss" else "#379d6c"
            ...
            #self.superframe.createSuperFrameText(text=match.rank_delta if "-" in match.rank_delta else "+"+match.rank_delta, x=x+delta[0], y=y+delta[1], anchor="c", font=fonttk("Refrigerator Deluxe", 9, "normal", italic=False), fill= RANK_FG.get(match.rank.lower(), "#B1C7F7"))    
            self.superframe.createSuperFrameText(text=match.timestamp, x=x+timestamp[0], y=y+timestamp[1], anchor="w", font=fonttk("Refrigerator Deluxe", 10, "normal", italic=False), fill=t_fg)
            ...
            self.superframe.createSuperFrameImage(img_key="kills_m", x=x+k, y=y+19 , anc="c")
            self.superframe.createSuperFrameImage(img_key="deaths_m", x=x+d, y=y+19 , anc="c")
            self.superframe.createSuperFrameImage(img_key="assists_m", x=x+a, y=y+19 , anc="c")
    ...
    def on_f8_pressed(self, tvar_value):
        ...
        if bUseRivalsDataNames:
            if bLiveDebug:
                ...
                p = filedialog.askopenfilename(title="Select a Live Match JSON file", initialdir=config.FullDebug_dir, filetypes=[("JSON files", "*.json")])
                try:
