...
class SpriteSheetAnimator:
    def __init__(
        ...
        icon_wrap = tk.Frame(master, width=s(frame_height), height=s(frame_height), bg=bg)
        ...
        self.canvas.pack(fill="both",side ="bottom",anchor="sw", expand=False)
        ...
        if not self.animated:
            ...
            try:
                ...
                if self.frame:
                    ...
                    self.frame_ne_id = self.canvas.create_image(s(frame_height), s(0), image=self.frame_ne_img, anchor="ne")
                if self.badge:
                    pass
                    add = 0
                    badge_name = f"_prof_badge{self.badge}"
                    self.badge_img = self.createOnlyImage(badge_name, bNearest=False, size=(s(34+add), s(34+add)))
                    self.badge_id = self.canvas.create_image(s(16), s(16), image=self.badge_img, anchor="center")
                ...
                if self.frame:
                    ...
                    self.frame_sw_id = self.canvas.create_image(s(0), s(frame_height-1), image=self.frame_sw_img, anchor="sw")
...
class PlayerFrame:
    ...
    def proficiency_handler(self,hours, hero):
            if hours > 100:
                return True, "gold", 4, hero
            elif hours > 60:
                return True, "gold", 3, hero
            elif hours > 48:    
                return False, "gold", 3, hero + "_l"
            elif hours > 35:
                return False, "purp", 2, hero + "_l"
            elif hours > 28:
                return False, 'purp', 1, hero + "_l"
            elif hours > 21:
                return False, False, 1, hero + "_l"
            elif hours > 15:
                return False, False, False, hero + "_l"
            else:
                return False, False, False, hero
    ...
    def _build_heroes(self):
        ...
        for idx1 in self.order:  # idx1 is 1..3
            ...
            if hero != "Unknown":
                if MANUAL_DEBUG != 1:
                    ...
                    if time_played_h is False:
                        ...
                        bAnimatedLord, frame, badge, hero_icon = self.proficiency_handler(time_played_h, hero)
                    else:
                        bAnimatedLord, frame, badge, hero_icon = self.proficiency_handlerAll(time_played_h, hero)
            ...
            hero_icon_frame2 = tk.Frame(
                ...
                padx=0,
                pady=0)
            hero_icon_frame2.pack(side="left",anchor="w",padx=(s(0), s(0)), pady=(s(0), s(0)))
            ...
            try:
                if bAnimatedLord:
                    ...
                    anim = SpriteSheetAnimator(
                        ...
                        borderwidth=2,
                        frame_height = 90,
                        size=(87, 87),   # optional: force icon size
                    )
    ...
    # stats canvas
        ...
        #canTags = ["text", "header", "bg", "games_group", "time_group","]
        hero_statstop.pack(side="left", padx=(0, 0), pady=(0, 0) )
