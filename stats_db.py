...
class StatsDB:
    ...
    def stat_percentile(
        ...
        *,
        ...
        # If a player_name is provided, exclude that player's row from the counts
        if player_name:
            total = cur.execute(
                f"SELECT COUNT(*) AS n FROM player_heroes WHERE hero_name=? AND {stat_key} IS NOT NULL AND LOWER(player_name) != ?;",
                (hero_name, player_name.lower()),
            ).fetchone()["n"]
        else:
            total = cur.execute(
        ...
        if direction == "desc":  # higher is better
            if player_name:
                better = cur.execute(
                    f"SELECT COUNT(*) AS n FROM player_heroes WHERE hero_name=? AND {stat_key} > ? AND LOWER(player_name) != ?;",
                    (hero_name, stat_value, player_name.lower()),
                ).fetchone()["n"]
            else:
                better = cur.execute(
    ...
    def overview_stat_percentile(
        ...
        *,
        ...
        # Exclude the provided player if present
        if player_name:
            total = cur.execute(
                f"""
                SELECT COUNT(*) AS n
                FROM player_overview
                WHERE {stat_key} IS NOT NULL AND LOWER(player_name) != ?;
                """,
                (player_name.lower(),),
            ).fetchone()["n"]
        else:
        ...
        if direction == "desc":  # higher is better
            if player_name:
                better = cur.execute(
                    ...
                    SELECT COUNT(*) AS n
                    FROM player_overview
                    WHERE {stat_key} > ? AND LOWER(player_name) != ?;
                    """,
                    (stat_value, player_name.lower()),
                ).fetchone()["n"]
            else:
                better = cur.execute(
                    f"""
