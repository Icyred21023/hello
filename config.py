...
ap3 = os.path.join(local_marvel_tracker_path, "config", "user_account_config.json")
...
if not os.path.exists(ap3):
    account_config_json = {
        "set": USER_NAME,
        "accounts": { USER_NAME: USER_UID}
    }
    helpers.save_json(ap3, account_config_json)

else:
    account_config_json = helpers.load_json(ap3)
    setname = account_config_json.get("set")
    if setname != USER_NAME:
        USER_NAME = setname
        USER_UID = account_config_json["accounts"].get(USER_NAME)
