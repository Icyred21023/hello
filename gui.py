...
def get_monitors():
    ...
    for index, (handle, hdc, rect) in enumerate(
        ...
        monitors.append({
            "index": index,
            "name": info.get("Device", f"Monitor {index + 1}"),
            "left": left,
            "top": top,
            "right": right,
            "bottom": bottom,
...
class App:
    def __init__(self, base_dpi_scale):
        ...
        self.selected_monitor_index = getattr(
            config,
            "monitor_index",
            0
        )
        ...
        if self.monitors:
            self.selected_monitor_index = max(
                0,
                min(self.selected_monitor_index, len(self.monitors) - 1)
            )
    ...
    def build_match_overview(self, parent, players):
        ...
        self.after_idle_tracked(
            lambda: self.force_geometry(w, h, x, y)
        )
    ...
    def on_f8_pressed(self, tvar_value):
        ...
        if search_by_name:
            ...
            if bUseRivalsDataNames:
                ...
                names = curlRivals.getLiveMatchNames()
